# works/Hyeong_Uk/test_dashboard/shorts_agent_tab.py
...
for d in [PERSONA_DATA_DIR, PERSONA_RESULT_DIR, PERSONA_VIDEO_DIR]:
...
def run_video_agent(
    ...
    # Streamlit secrets는 subprocess에 자동 전달되지 않으므로 환경변수로 주입
    env["GOOGLE_CLOUD_PROJECT"] = st.secrets.get("GOOGLE_CLOUD_PROJECT", os.getenv("GOOGLE_CLOUD_PROJECT", ""))
    env["GOOGLE_CLOUD_REGION"]  = st.secrets.get("GOOGLE_CLOUD_REGION", os.getenv("GOOGLE_CLOUD_REGION", "us-central1"))
...
def _inject_shorts_agent_css():
    st.markdown(
        ...
        [data-testid="stMetricValue"]{color:#111827!important;font-size:20px!important;font-weight:900!important}
        </style>
        """,
        unsafe_allow_html=True,
    )
...
def _render_shorts_agent_header():
    st.markdown(
        """
...
def _refresh_shorts_agent_secrets():
    global GOOGLE_CLOUD_PROJECT, GOOGLE_CLOUD_REGION, GEMINI_MODEL, YOUTUBE_API_KEY
    GOOGLE_CLOUD_PROJECT = st.secrets.get("GOOGLE_CLOUD_PROJECT", os.getenv("GOOGLE_CLOUD_PROJECT", ""))
    GOOGLE_CLOUD_REGION = st.secrets.get("GOOGLE_CLOUD_REGION", os.getenv("GOOGLE_CLOUD_REGION", "us-central1"))
    GEMINI_MODEL = st.secrets.get(
        "GEMINI_PROMPT_MODEL",
        st.secrets.get("GEMINI_MODEL", os.getenv("GEMINI_PROMPT_MODEL", "gemini-2.5-flash-lite")),
    )
...
def render_shorts_agent_tab():
    ...
    st.markdown("<hr style='border-color:#e5e7eb;margin:24px 0 16px'>", unsafe_allow_html=True)
    st.markdown(
        '<div class="sec-title"><span class="tbar" style="background:#6b7280"></span>보고서 다운로드</div>',
        unsafe_allow_html=True,
    )
